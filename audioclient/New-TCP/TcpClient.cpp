...
int TcpClient::connectServer(const char *hostIP, unsigned short hostPort, bool bKeepLive) {
    unsigned int ip = inet_addr(hostIP);
	return connectServer(ip, hostPort, bKeepLive);
}
//---------------------------------------------------------------------------

//typedef uint32_t in_addr_t;
int TcpClient::connectServer(unsigned int hostIP, unsigned short hostPort, bool bKeepLive) {
	char strErr[300];
	int iRet;
...
ssize_t TcpClient::sendData(void* data0, size_t length) {
